...
import { getApiContext } from './api-hooks';

const apiContext = getApiContext();

When('the user sends a POST request to create an order with product ID {int} quantity {int}', async (productId: number, quantity: number) => {
  try {
...
When('the user sends a PATCH request to update order {int} status to {string}', async (orderId: number, status: string) => {
  try {
    await apiContext.orderApi.updateOrderStatus(orderId, status as any);
  } catch (error) {
    // Error is stored in apiContext
  }
});

When('the user sends a DELETE request to cancel order {int}', async (orderId: number) => {
  try {
